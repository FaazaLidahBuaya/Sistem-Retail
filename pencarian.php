...
// Ambil kata kunci dari URL (yang diketik user)
$keyword = $_GET["keyword"];

// Ambil data produk yang NAMANYA atau DESKRIPSINYA mengandung kata kunci
// Pakai tanda persen (%) agar mencari kata yang "mirip" di depan atau belakang
$semuadata = array();
$ambil = $koneksi->query("SELECT * FROM products WHERE name LIKE '%$keyword%' OR deskripsi LIKE '%$keyword%'");
...
<head>
    ...
    <title>Pencarian: <?= $keyword; ?> - Faaza Store</title>
...
<body>
    ...
    <nav class="navbar navbar-expand-lg bg-white py-3 shadow-sm sticky-top">
        <div class="container">
            ...
            <form action="pencarian.php" method="GET" class="input-group w-50 mx-auto search-box">
                <input type="text" name="keyword" class="form-control" value="<?= $keyword ?>" placeholder="Cari barang apa...">
    ...
    <div class="container mt-4 mb-5" style="background-color: #b0b0b0; padding: 30px; border-radius: 20px;">
        ...
        <h4 class="fw-bold mb-4 text-white">Hasil Pencarian: "<?= $keyword ?>"</h4>
        
        <?php if (empty($semuadata)): ?>
            <div class="alert alert-danger text-center p-5">
                <h3><i class="fas fa-search-minus mb-3" style="font-size: 50px;"></i></h3>
                <p class="fw-bold">Yah, produk "<?= $keyword ?>" nggak ketemu...</p>
                <p>Coba cari kata kunci lain ya!</p>
                <a href="index.php" class="btn btn-dark mt-2">Kembali ke Home</a>
            </div>
        <?php else: ?>
            <div class="row g-3">
